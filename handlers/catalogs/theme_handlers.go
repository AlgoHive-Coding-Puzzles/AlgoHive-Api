...
func GetThemesFromCatalog(c *gin.Context) {
    ...
    if err == nil {
        err = database.REDIS.Set(ctx, cacheKey, themesJSON, config.RedisExpiMin*time.Minute).Err()
        if err != nil {
...
func GetThemeDetailsFromCatalog(c *gin.Context) {
    ...
    if err == nil {
        err = database.REDIS.Set(ctx, cacheKey, themeDetailsJSON, config.RedisExpiMin*time.Minute).Err()
        if err != nil {
            // Continue even if caching fails
        } else {
